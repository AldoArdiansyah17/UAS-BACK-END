...
if (isset($_GET['id']) && !empty($_GET['id'])) {
    ...
    if ($result->num_rows > 0) {
        ...
        $sqlAll = "SELECT * FROM nilai WHERE mahasiswa = '$mahasiswa'";
...
if (isset($_GET['delete'])) {
    $id = $_GET['delete'];
    $sql = "UPDATE nilai SET nilai = 0 WHERE id = $id";
    if ($conn->query($sql) === TRUE) {
        // Mengambil id mahasiswa untuk redirect
        $sqlMahasiswa = "SELECT mahasiswa FROM nilai WHERE id = $id LIMIT 1";
